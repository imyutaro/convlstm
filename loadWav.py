...
def show_input(data_name, label_list, data_shape):
    ...
    print('{:=^60}'.format(''))
    print('count 0 : {0:02d}'.format(np.count_nonzero(1-label_list)))
    print('count 1 : {0:2d}'.format(np.count_nonzero(label_list)))
    print('{:-^60}'.format(''))
    print('shape:',data_shape)
    print('{:-^60}\n'.format(''))

def form(file_path, data_format):
    ...
    if data_format=='power':
        ...
        data = librosa.feature.mfcc(y=x, sr=fs ,n_mfcc=128)
...
def dim2(dir_path=None, fav_label=None):
    ...
    if fav_list is None:
        return print('fav_label path is None\n')
...
def dim3(dir_path=None, FRAME_NUM=700, fav_label=None, data_format='power'):
    ...
    for file_name in files:
        ...
        if ext == '.wav':
            ...
            if audio_list.shape[1] < FRAME_NUM:
                for _ in range(math.ceil(FRAME_NUM/audio_list.shape[1])):
                    audio_list=np.hstack((audio_list,audio_list))
...
def time_dim3_overlap(dir_path=None, FRAME_NUM=599, fav_label=None, OVERLAP=True, data_format='power'):
    ...
    overlap=4 # 1/2でoverlapしていく
    FRAME_NUM = int(688*2)
    WINDOW = int(688)
    stride = int(WINDOW/overlap) #=>172
    ...
    for file_name in files:
        ...
        if ext == '.wav':
            ...
            if audio_list.shape[1] < FRAME_NUM:
                for _ in range(math.ceil(FRAME_NUM/stride)):
                    audio_list=np.hstack((audio_list,audio_list))
            #print(audio_list.shape)
    ...
    data_name = data_name.reshape(data_name.shape[0],1)
    show_input(data_name=data_name, label_list=label_list, data_shape=input_data.shape)
    #input_data = np.reshape(input_data, (input_data.shape[0], input_data.shape[1], input_data.shape[3], input_data.shape[2]))

    return input_data, label_list, data_name

def for_prediction(input_shape, dir_path=None, fav_label=None, OVERLAP=False, data_format='mfcc', SR=44100):
    ...
    for file_name in files:
        ...
        if ext == '.wav':
            ...
            audio_list = form(file_path=dir_path+file_name, data_format=data_format, sr=SR)
